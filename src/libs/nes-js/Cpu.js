...
Object.assign(Cpu.prototype, {
  ...
  load2BytesFromZeropage: function (address) {
    return (
      this.ram.load(address & 0xff) | (this.ram.load((address + 1) & 0xff) << 8)
    );
  },
  ...
  loadWithAddressingMode: function (op) {
    if (op.mode.id === this.ADDRESSINGS.ACCUMULATOR.id) return this.a.load();
  ...
  disassembleROM: function () {
    ...
    while (pc < 0x4010) {
      ...
      str += this.dumpMemoryAddressingMode(op, rom, (pc + 1) & 0xffff) + ' ';
  ...
  dump: function () {
    ...
    try {
      ...
      buffer +=
        op.instruction.name +
        ' ' +
        this.dumpMemoryAddressingMode(op, this, (this.pc.load() + 1) & 0xffff) +
        ' ';
    } finally {
      while (buffer.length < 90) {
        buffer += ' ';
    ...
    switch (op.mode) {
      ...
      case this.ADDRESSINGS.ABSOLUTE:
        ...
        if (ramDump) {
          buffer +=
            '(' +
            Utility.convertDecToHexString(mem.load(address, true), 2) +
            ')';
        }
        break;
      ...
      case this.ADDRESSINGS.INDIRECT:
        ...
        if (ramDump) {
          ...
          buffer +=
            '(' +
            Utility.convertDecToHexString(mem.load(address2, true), 2) +
            ')';
          buffer += ')';
        }
        break;
      ...
      case this.ADDRESSINGS.ACCUMULATOR:
        if (ramDump) {
          buffer +=
            'A(' + Utility.convertDecToHexString(this.a.load(), 2) + ')';
        }
        break;
